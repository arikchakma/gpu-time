...
const dataPath = `data/synth/${run}/heldout.jsonl`;
if (!existsSync(dataPath)) {
  const config = JSON.parse(
    await readFile(`training/runs/${run}/report.json`, "utf8"),
  ).config;
  execFileSync(
    "uv",
    [
      ...
      "training",
      "python",
      `training/runs/${run}/source/training/generate.py`,
      "--count",
...
try {
  ...
  const result = await page.evaluate(async (texts) => {
    ...
    const binary = async (name: string) =>
      (await fetch(`/training/parity.${name}.bin`)).arrayBuffer();
    ...
    const { boundaryThreshold = 0 } = await (
      await fetch("/training/parity.json")
    ).json();
  ...
  if (
    ...
    throw new Error(
      "Browser parity or device recovery failed; see training/parity-gpu.json",
    );
} finally {
  await browser.close();
  await server.close();
