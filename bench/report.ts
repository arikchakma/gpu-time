...
const read = async (name: string) =>
  JSON.parse(await readFile(`bench/results/${name}.json`, "utf8"));
...
const model = JSON.parse(await readFile("training/export-report.json", "utf8"));
const direct = await read("direct-results");
const gpu = JSON.parse(await readFile("training/parity-gpu.json", "utf8"));
const external = await read("recognizers-development");
const semantic = JSON.parse(
  await readFile("training/semantic-evaluation.json", "utf8"),
);
if (
...
await writeFile("bench/results/REPORT.md", lines.join("\n"));
console.log("Wrote bench/results/REPORT.md and summary.json");
