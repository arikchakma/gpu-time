...
import { matches, type Expected } from "./recognizers-values.js";
interface Example {
  id: string;
  family: string;
  text: string;
...
const cases: Example[] = (
  await readFile("data/external/recognizers/cases.jsonl", "utf8")
)
  .trim()
  .split("\n")
...
await writeFile(
  `bench/results/recognizers-${split}.json`,
  JSON.stringify(
    {
      model: JSON.parse(await readFile("training/export-report.json", "utf8"))
        .artifactSha256,
      corpus: JSON.parse(
        await readFile("data/external/recognizers/manifest.json", "utf8"),
      ),
      split,
      total: cases.length,
      correct,
