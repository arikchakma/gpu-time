...
timeline = json.loads(Path("video/output/motion-timeline.json").read_text())
...
for name, data in [("gpu-time-music-bed.wav", bed), ("gpu-time-music-continuous.wav", continuous_bed),
    ...
    with wave.open("video/output/"+name, "wb") as output:
...
Path("video/output/sound-cues.json").write_text(json.dumps(report, indent=2))
