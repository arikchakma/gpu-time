...
import { decodeWeights } from '../../src/model/decode.ts';
...
assert.deepEqual(Array.from(prediction.labels), tagged.tokens.map(token => token.label));
...
const model = tokens.map((token, i) => {
  const trace = Object.fromEntries(Object.entries(prediction.trace!).map(([key, values]) => [key,
    Array.from(key === 'pooled' || key === 'context' ? values : values.slice(i*32, (i+1)*32)),
  ]));
  ...
  const headGate = Array.from({ length: 16 }, (_, row) => {
    ...
    let v = gateBias[row];
    for (let k=0; k<32; k++) { v += x[k]*matrix[row*64+k]; v += x[32+k]*matrix[row*64+32+k]; }
  ...
  const headHidden = Array.from({ length: 64 }, (_, row) => {
    ...
    for(let k=0;k<32;k++){v+=x[k]*matrix[row*80+k];v+=x[32+k]*matrix[row*80+32+k];}
    for(let k=0;k<16;k++)v+=headGate[k]*matrix[row*80+64+k];
    return Math.tanh(Math.fround(v));
  });
  const logits = Array.from(prediction.logits!.slice(i*weights.roleClasses,(i+1)*weights.roleClasses));
  if(token.kind !== 3) {
    for(let slot=0;slot<weights.roleClasses;slot++){
      const reconstructed=tensors.get('output_bias')![slot]+dot(headHidden,'output_weight',slot,64);
      assert(Math.abs(reconstructed-logits[slot])<0.00001, `Head trace mismatch: ${i}:${slot}`);
    }
  }
  return {...token, label:LABELS[prediction.labels[i]], featureRows:featureRows(token.features), ...trace,
    headGate, headHidden, logits, boundaryLogit:prediction.boundaryLogits![i], clauseStart:Boolean(prediction.clauseStarts[i])};
});
const report = JSON.parse(readFileSync('training/export-report.json', 'utf8'));
...
writeFileSync('video/current/data.json', JSON.stringify({
  provenance:{source:'src/index.ts + src/model/cpu.ts diagnostic trace', sourceSha256:hash.digest('hex'), checkpointSha256:report.checkpointSha256, artifactSha256:report.artifactSha256},
  text, context, parameters:report.parameters, architecture:{embeddingRows:weights.featureRows, channels:32, headGate:16, headHidden:64, roleSlots:weights.roleClasses, namedRoles:LABELS.length, boundaryThreshold:weights.boundaryThreshold},
  labels:Array.from({length:weights.roleClasses},(_,i)=>LABELS[i]??`reserved_${i}`),
  model:model.filter(t=>t.kind!==3), allTokens:model,
  expansion:{occurrences:result.occurrences.map(t=>({...t,instant:new Date(t.start).toISOString()}))},
  rules:[{dtstart:rule[0],dtend:rule[1],rrule:rule[2]}], result,
},null,2)+'\n');
