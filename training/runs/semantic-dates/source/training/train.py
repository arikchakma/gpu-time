...
def main():
    ...
    sources = ["training/model.py", "training/train.py", "training/generate.py", "training/semantic.py", "training/signature.py", "training/featurize.ts", "src/tokenizer.ts", "src/labels.ts"]
    source_hashes = {}
    for name in sources:
    ...
    for epoch in range(args.epochs):
        ...
        for batch_index, indices in enumerate(batches):
            ...
            learning_rate = (1e-4 + (args.learning_rate - 1e-4) * (1 + math.cos(math.pi * progress)) / 2) * min(1, step / args.warmup_steps)
