...
def quantize_int6(weight: Tensor) -> Tensor:
    scale = (weight.detach().abs().max() / 31).clamp_min(1e-8)
    quantized = (weight / scale).round().clamp(-31, 31) * scale
...
class TimeTagger(nn.Module):
    def __init__(self) -> None:
        ...
        assert sum(parameter.numel() for parameter in self.parameters()) == 32953

    def weight(self, name: str) -> Tensor:
        value = getattr(self, name)
        return quantize_int6(value) if self.qat else value
    ...
    def forward(self, rows: Tensor, valid: Tensor, neighbors: Tensor) -> tuple[Tensor, Tensor]:
        row_mask = (rows != PADDING_ROW).unsqueeze(-1)
        embedded = F.embedding(rows.clamp_max(FEATURE_ROWS - 1), self.weight("embedding"))
        ...
        for side in range(2):
            ...
            gathered = embedded.gather(1, indices.clamp_min(0).unsqueeze(-1).expand(-1, -1, HIDDEN))
            encoded = encoded + gathered * (indices >= 0).unsqueeze(-1) * self.weight("neighbor_weights")[side]
        ...
        combined = self.store(torch.tanh(encoded + self.linear(torch.cat((forward, backward), dim=-1), "combine")))
        ...
        if self.record_trace:
            self.trace = {name: value.detach() for name, value in {"embedded": embedded, "encoded": encoded, "gate": gate, "candidate": candidate, "forward": forward, "backward": backward, "combined": combined, "pooled": pooled, "context": context}.items()}
