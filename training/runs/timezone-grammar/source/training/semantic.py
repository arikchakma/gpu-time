...
ZONES = {
    ...
    "Etc/GMT+6": ["MDT"],
    "Etc/GMT+8": ["PST"],
    "Asia/Dhaka": ["Dhaka time", "Asia/Dhaka"],
...
def sample_general(family: str, rng: random.Random) -> dict:
    ...
    if family == "timezone":
        return {
            "time": {"start": sample_clock(rng)},
            "timeZone": rng.choice(list(ZONES)),
        }
...
def render_general(clause: dict, sentence: Sentence, style: int) -> None:
    ...
    if clause.get("timeZone"):
        sentence.add(sentence.rng.choice(ZONES[clause["timeZone"]]), "TZ")
