...
class Sentence:
    ...
    def quantity_unit(
        self, units: list[str] = UNITS, amount: int | None = None
    ) -> None:
        ...
        sentence.add(
            rng.choice(
                [
                    ...
                    "let us meet",
                    "deadline:",
                    "room 10 is booked for",
                    "call 3 people about",
                    "I may be available",
                    "please schedule",
                    "our second discussion is",
                    "Alex asked about",
                ]
            )
        )
...
def generate(
    ...
    with path.open("w") as output:
        ...
        while index < count:
            ...
            invalid_zone = split != "heldout" and rng.random() < 0.03
            if invalid_zone:
                spec = None
                sentence = Sentence(rng)
                sentence.clause()
                sentence.clock(rng.choice([0, 2, 3]))
                sentence.add(
                    rng.choice(["CST", "IST", "BST", "cst", "ist", "bst"]), "TZ"
                )
                template = "invalid/ambiguous-timezone"
            elif spec:
                sentence = Sentence(rng)
            ...
            if invalid_zone:
                row["schedule"] = None
                row["diagnostics"] = ["ambiguous-timezone"]
            key = fingerprint(row)
            if key in reserved:
                rejected += 1
                continue
