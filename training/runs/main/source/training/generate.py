...
class Sentence:
    ...
    def add(self, text: str, label: str = "O", separator: str = " ") -> None:
        ...
        boundary = self.pending_clause and label != "O"
        if boundary:
            self.pending_clause = False
    ...
    def clock(self, style: int | None = None) -> None:
        style = self.rng.randrange(7) if style is None else style
        if style == 0:
    ...
    def window(self, variant: int) -> None:
        ...
        self.clock(self.rng.choice([2, 3, 5, 6]))
        ...
        self.clock(self.rng.choice([0, 2, 3, 5]))
...
def render(family: int, variant: int, rng: random.Random) -> Sentence:
    ...
    if family == 0:  # A weekday list shares a clock or a window.
        ...
        sentence.add(rng.choice(["-", "–", "through"]), "RANGE_END")
...
def generate(path: Path, count: int, seed: int, split: str) -> dict:
    ...
    return {"generatorSha256": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(), "sequences": count, "seed": seed, "split": split, "templates": sorted(templates), "families": dict(families), "spanCounts": dict(span_counts)}
...
if __name__ == "__main__":
    ...
    report = generate(args.out, args.count, args.seed, args.split)
