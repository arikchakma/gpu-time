...
def prepare(split: str, count: int, seed: int, directory: Path) -> Dataset:
    ...
    subprocess.run(["uv", "run", "--project", "training", "python", "training/generate.py", "--count", str(count), "--seed", str(seed), "--split", split, "--out", f"{prefix}.jsonl"], cwd=ROOT, check=True, stdout=subprocess.DEVNULL)
...
def main():
    ...
    if set(training.manifest["templates"]) & set(heldout.manifest["templates"]):
    ...
    for epoch in range(args.epochs):
        ...
        checkpoint = {"model": {name: value.detach().cpu() for name, value in model.state_dict().items()}, "optimizer": optimizer.state_dict(), "config": vars(args), "epoch": epoch + 1, "metrics": metrics, "tokensSeen": tokens_seen}
        ...
        report = {
            ...
            "sourceHashes": {name: hashlib.sha256((ROOT / name).read_bytes()).hexdigest() for name in ["training/model.py", "training/train.py", "training/generate.py", "training/featurize.ts", "src/tokenizer.ts", "src/labels.ts"]},
