...
def prefix(rng: random.Random) -> str:
    ...
    if rng.random() < 0.25:
        return f"{rng.choice(['please', 'can we', 'we should', 'remember to'])} {rng.choice(ACTIONS)} the {rng.choice(OBJECTS)} for"
...
def sentence(rng: random.Random) -> str:
    if rng.random() < 0.4:
        ...
        return rng.choice(
            [
                f"Please {action} this {item}.",
                f"We keep this in the {place}.",
                f"You can put this in your {place}.",
                f"The {item} needs extra space.",
                f"We should {action} the {item} in our {place}.",
                f"Our {item} is ready for review.",
            ]
        )
    if rng.random() < 0.12:
        subject = rng.choice(
