...
def clock(s, mode=None, hour=None):
    ...
    if mode == "spoken" and r.random() < 0.25:
        m = 0
    if mode == "fraction":
        m = r.choice([15, 30, 45])
        subtract = m == 45
...
def render_base(s, reserved=False, family=None):
    r = s.rng
    ...
    elif family == "clock-range":
        end = r.randint(2, 12)
        start = r.randint(1, end - 1)
        marker = r.choice(["am", "pm"])
        s.add(r.choice(["between", "from"]), "RANGE_START")
        s.add(str(start) if r.random() < 0.5 else words(start), "HOUR")
        s.add(r.choice(["and", "to", "-"]), "RANGE_END")
        s.add(str(end), "HOUR")
        s.add(marker, "MERIDIEM", "")
        # The nearest preceding clock inherits its partner's meridiem.
        finish = end % 12 + (12 if marker == "pm" else 0)
        begin = (finish - (end - start)) % 24
        clause = {
            "time": {
                "start": {"hour": begin, "minute": 0},
                "end": {"hour": finish, "minute": 0},
            }
        }
    elif family == "trailing-days":
        first = r.randint(0, 4)
        last = r.randint(first + 1, 6)
        start = r.randint(7, 11)
        end = r.randint(1, 6)
        if r.random() < 0.5:
            s.add("from", "RANGE_START")
        s.add(str(start), "HOUR")
        s.add(r.choice(["to", "-", "through"]), "RANGE_END")
        s.add(str(end), "HOUR")
        if r.random() < 0.5:
            s.add("pm", "MERIDIEM")
        if r.random() < 0.5:
            s.add("on", "GLUE")
        s.add(DAYS[first] if r.random() < 0.5 else DAYS[first][:3], "WEEKDAY")
        s.add(r.choice(["through", "-", "to"]), "RANGE_END")
        s.add(DAYS[last] if r.random() < 0.5 else DAYS[last][:3], "WEEKDAY")
        clause = {
            "time": {
                "start": {"hour": start, "minute": 0},
                "end": {"hour": end + 12, "minute": 0},
            },
            "recurrence": {
                "freq": "weekly",
                "interval": 1,
                "byDay": DAY_CODES[first : last + 1],
            },
        }
...
def render(s, reserved=False, family=None):
    spec = render_base(s, reserved, family)
    # English case variants keep all UTF-16 span offsets unchanged.
    if not reserved:
        casing = s.rng.randrange(5)
        if casing == 0:
            s.text = s.text.upper()
        elif casing == 1:
            s.text = s.text.lower()
