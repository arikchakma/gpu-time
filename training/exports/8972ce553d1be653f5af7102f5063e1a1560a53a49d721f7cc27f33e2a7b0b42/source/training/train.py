...
def prepare(split: str, count: int, seed: int, directory: Path) -> Dataset:
    ...
    subprocess.run(
        ["bun", "training/featurize.ts", f"{prefix}.jsonl", str(prefix)],
        cwd=ROOT,
        check=True,
...
def main():
    ...
    if args.init:
        ...
        embedding = initial["model"]["embedding"]
        if embedding.shape[0] == 324 and args.feature_rows == 580:
            # Restore canonical feature rows with identical initial predictions.
            # Previously discarded hash rows start at zero and can now learn.
            mapping = TimeTagger(324).row_map[:580]
            initial["model"]["embedding"] = F.pad(embedding, (0, 0, 0, 1))[mapping]
    ...
    sources = [
        ...
        "training/train.py",
        "training/generate.py",
        "training/semantic.py",
        "training/background.py",
        "training/signature.py",
        "training/featurize.ts",
        "src/tokenizer.ts",
        "src/labels.ts",
    ]
    source_hashes = {}
