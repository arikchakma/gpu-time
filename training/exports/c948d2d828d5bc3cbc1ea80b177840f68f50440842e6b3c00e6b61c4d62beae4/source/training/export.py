...
def export(
    ...
    for name, parameter in reference.named_parameters():
        ...
        if row_scales and values.ndim == 2:
            ...
            exponents = np.clip(np.ceil(np.log2(row_max / np.float32(scale * maximum))), -16, 0).astype(np.int8)
        ...
        segments.append(
            {
                ...
                "scale": float(scale),
                **({"rowScales": "".join(chr(65 - int(value)) for value in exponents.flat)} if exponents is not None else {}),
            }
        )
