...
- Keep `natural.py::RESERVED` unreachable from the training carrier grammar. Those phrases are the only honest measure of unseen prose, and the headline number means nothing once they leak into training.
- Timezone stays out of the model. Calendar arithmetic has exact answers; resolve it in TypeScript.
- The honest headline accuracy number is **993/1000 on unseen carriers**. Held-in generated scores share rendering families with training and must not be quoted as language accuracy. See `MODEL_CARD.md`.
- Train with `--storage f32 --feature-rows 324 --init runs/<promoted>/best.pt`. The defaults are f16 and a cold start; f16 costs 225x the CPU/PyTorch parity error and a cold start loses rare terse forms.
