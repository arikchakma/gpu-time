...
import { normalize } from "../bench/recognizers-values.js";

const root = "data/external/recognizers";
...
const previous =
  existsSync(`${root}/manifest.json`) && !process.argv.includes("--refresh")
...
await writeFile(`${root}/LICENSE`, license);
const records = [];
const skipped = [];
const sources = [];
for (const file of files) {
  ...
  await writeFile(`${root}/raw/${file}.json`, source);
