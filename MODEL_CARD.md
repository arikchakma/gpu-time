...
The package embeds checkpoint `step5-keepo-crf3`, epoch index 16, with artifact SHA-256 `b1dfa50dc6fa5cca7f710b06bf64e48af0ec138852d5ddf0fb093e76d9e9018e`. It has 34,553 parameters, 580 embedding rows, and 40 role slots (35 named roles plus five reserved). The extra parameters over the previous 32,953 are a 40x40 role transition matrix: this is the first promoted checkpoint trained as a linear-chain CRF (`train.py --transitions`) and decoded by Viterbi. Weights use 6-bit symmetric per-tensor quantization, with f32 intermediate calculations. The active report records 19,089 Brotli bytes for the weights, six checkpoints, and 614,862,955 training tokens.

The model predicts one role per token, such as hour, weekday, quantity, recurrence marker, or filler. A separate boundary score splits the input into expressions at threshold 1.25, fitted by `calibrate.py` on the development splits. The `CLOCK_OFFSET` role represents half-hour and quarter-hour clock arithmetic.
...
The active weights have a six-checkpoint history with 614,862,955 training tokens. The final run starts from `step5-keepo-crf` and draws 300,000 fresh examples per epoch. A fresh model can lose terse forms that earlier checkpoints learned.
...
- **Hand-authored chat gold: 270/330 for `step5-keepo-crf3`, unchanged from `step3-580b`.** Per family: question 26/27, tatoeba 64/86 (was 69/86), calendar 45/51 (was 46/51), abbrev 25/28, correction 14/16 (was 13/16), prose 20/22 (was 18/22), relative 19/22, recognizers 46/62 (was 44/62), negation 11/16 (was 10/16). Chat is hand-written for this project, so it is a development set, not held-out data.
- **Non-temporal negatives: 186/192, and 128/132 on the original 132 rows, up from 111/132.** `negatives.jsonl` grew by sixty rows (133-192) covering measurement durations after a completion verb, ordinals on nouns, numbered things, scores, numeric ranges, ages, percents, month names used as people, fractions and vague counts. On the previous `step3-580b` weights that 192-row file scored 150/192. Six negatives still fail and `packages/core/test/grammar-model.test.ts` keeps them as running `it.fails` cases.
- **Prose gold: 70/73, down from 73/73.** The three rows are one sentence in three casings: "The meeting is scheduled for next week" labels `for` as `DUR`, so the clause compiles to null. It abstains with a diagnostic rather than returning a wrong date, and it is listed in the same `it.fails` set. This is the one positive gold row the negatives work cost.
